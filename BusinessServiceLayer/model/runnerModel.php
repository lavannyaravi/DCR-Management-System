...
class runnerModel{
    public $R_Mail,$R_Password,$R_Name,$R_Phone,$R_LicienseNo,
    $R_Address,$R_image;

    // get email and password for runner to login 
    function loginRunner(){
        if(isset($_POST['R_Mail'])){
        $sql = "select * from rider where R_Mail=:R_Mail AND R_Password=:R_Password limit 1";
           //$sql = "select * from runner where RunnerEmail=:RunnerEmail";
        $args = [':R_Mail'=>$this->R_Mail, ':R_Password'=>$this->R_Password];
    ...
    function registerRun(){
        ...
        $sql = "insert into rider (R_Name,R_Mail,R_Password,R_Phone,R_LicienseNo,
                 R_Address,R_image)

        value(:R_Name, :R_Mail, :R_Password,  :R_Phone, :R_LicienseNo, :R_Address, :R_image)";
		
		 $args = [':R_Name'=>$this->R_Name, ':R_Mail'=>$this->R_Mail, ':R_Phone'=>$this->R_Phone,
